...
class SalesModel extends CI_Model {
    ...
    */
    ...
    public function paid_amount_list($limit,$start,$param)
    {
		
       $date= date('Y-m-d');
	   $where=" ";

	  //echo $param['status']; exit();

       if(isset($param['todate'])&& $param['todate']!='')
        {
        	$todate=date('Y-m-d',strtotime($param['todate']));

            $where .= " AND DATE(order_date) <= '".$todate."' ";
        }
        if(isset($param['fromdate'])&& $param['fromdate']!='')
        {
        	$fromdate=date('Y-m-d',strtotime($param['fromdate']));
            $where .= " AND DATE(order_date) >= '".$fromdate."' ";
        }
       ...
       //echo"<pre>";print_r($data); exit();
        ...
        public function due_amount_list($limit,$start,$param)
    {
