    ...
    **/
    ...
    function record_count($param)
    {
		 ...
		 if(isset($param['name']) && $param['name']!='')
		{
		 ...
		 if(isset($param['lname']) && $param['lname']!='')
		{
			
			 $where .= " AND user.lastname like '%".$param['lname']."%' ";
		}
		//order_id
    ...
    public function all_data_list($limit,$start,$param)
    {
       ...
       if(isset($param['from_date']) && $param['from_date']!='')
	    ...
	    if(isset($param['status']) && $param['status']!='')
		{
		    ...
		    $sql="SELECT  orders.*,user.name,user.lastname,user.email,user.phone   FROM  kf_order orders left join kf_user user on user.user_id=orders.user_id  WHERE 1    $where  ORDER BY  orders.order_id DESC  LIMIT ".$start.",".$limit ;  	   
